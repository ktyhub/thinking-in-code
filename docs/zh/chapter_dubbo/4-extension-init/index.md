...
![4-2-arch.png](/img/chapter_dubbo/4-2-arch.png)
...
![在这里插入图片描述](/img/chapter_dubbo/4-3-ext.png)
...
| **ExtensionDirector**               | ExtensionDirector是一个作用域扩展加载程序管理器。ExtensionDirector支持多个级别，子级可以继承父级的扩展实例。查找和创建扩展实例的方法类似于Java classloader。                                                                                                           |
| **ExtensionScope**                  | 扩展SPI域,目前有FRAMEWORK,APPLICATION,MODULE,SELF                                                                                                                                                                       |
| **ExtensionLoader**                 | 	 ApplicationModel、DubboBootstrap和这个类目前被设计为单例或静态（本身完全静态或使用一些静态字段）。因此，从它们返回的实例属于process或classloader范围。如果想在一个进程中支持多个dubbo服务器，可能需要重构这三个类。                                                                            |
| **ExtensionPostProcessor**          | 在扩展初始化之前或之后调用的后处理器。                                                                                                                                                                                               |
| **LoadingStrategy**                 | 扩展加载策略,目前有3个扩展加载策略分别从不同文件目录加载扩展                                                                                                                                                                                   |
