...
• 字段（Field）：属性标识（如`author`）

• 字段值（Value）：具体信息（如`Jack`）
...
|----------------|----------------------------|------------------------|  
| 来源 | 系统预定义 | 用户手动创建 |  
| 字段示例 | `document_name`/`uploader`| 自由定义（如`category`） |  
| 修改权限 | 启用后不可修改 | 可编辑/删除字段 |  
| 启用方式 | 需手动启用 | 创建后自动生效 |
...
| 操作场景 | 影响范围 | 结果 |  
|---------------------------|--------------------------------|------------------------|  
| 知识库管理页删除字段 | 全局删除字段及所有文档中的值 | 字段从系统中彻底消失 |  
| 批量编辑弹窗删除字段 | 仅删除选中文档的字段值 | 字段保留，值计数变化 |  
| 单文档页删除字段 | 仅删除当前文档的字段值 | 字段保留，值计数变化 |

---

**5. 核心价值与应用**  
• 搜索优化：通过字段快速筛选文档（如`language=英语`）
...
• 自动化场景：
  ...
  ```python
  # 示例：自动标记上传者元数据
  if document.uploader == "admin":
      set_metadata("security_level", "high")
  ```  

• 数据治理：统一规范文档属性（如强制`category`字段）
...
> 完整API文档：参考[通过API维护元数据](链接)

此功能实现知识库的精细化管控，尤其适合企业级知识治理场景，通过结构化标签体系释放非结构化数据价值。

### 7.4.5 在应用内集成知识库
---

**在应用内集成知识库**
目的：将知识库作为外部知识源接入大模型，提升回答准确性。适用于所有Dify应用类型（如聊天助手、工作流等）。

**关键操作流程**

1. 关联知识库  
   • 进入应用工作室 → 创建/选择应用（如聊天助手）→ 上下文设置 → 添加已有知识库。
...
2. 召回策略设置  
   • 多路召回模式：同时检索所有关联知识库，通过Rerank策略合并最优结果。

   • 两种配置方式：

   ◦ 权重设置（无需额外成本）：

       ◦ 纯语义检索（权重1）：适合跨语言或模糊查询。

       ◦ 纯关键词检索（权重1）：适合精确术语匹配。

       ◦ 自定义权重：调整语义/关键词比例（如语义0.7 + 关键词0.3）。

   ◦ Rerank模型（需API成本）：

       调用外部模型（如Cohere、Jina AI）对混合内容重排序，适合复杂知识库场景。
   • 可调参数：

   ◦ `TopK`：控制返回文本片段数量（值越高召回越多）。

   ◦ `Score阈值`：筛选相似度分数（值越高召回越少）。


3. 元数据筛选  
   • 适用场景：按文档标签、类别、时间等元数据精确过滤内容。
   ...
   ◦ 手动模式（推荐复杂场景）：
       ...
       ◦ 添加条件：选择元数据字段（如字符串、数字、时间）。

       ◦ 设置逻辑关系（AND/OR）。

       ◦ 指定筛选值（常量或变量）：

         ▪ 字符串：支持精确匹配（`is`）、包含（`contains`）、前缀（`starts with`）等。

         ▪ 数字：支持范围（`>`, `<`）、等于（`=`）等。

         ▪ 时间：支持早于（`before`）、晚于（`after`）等。


4. 查看已关联应用  
   • 知识库信息栏显示关联应用数量，悬停可查看列表并快速跳转。
...
1. 如何选择召回策略？  
   • 精确术语 → 关键词权重1；跨语言/模糊查询 → 语义权重1；复杂场景 → Rerank模型。

2. 为何无法调整权重？  
   • 多个知识库的嵌入模型不一致时，需统一设置或启用Rerank模型。

3. “高质量”索引模式  
   • 经济模式不支持权重设置，需切换至高质量模式。

---

图示说明：  
• 网页包含多张配置界面截图（如召回设置、元数据筛选弹窗），需实操参考。

• 提供Rerank模型技术流程图（多知识库检索→合并→重排序→输出）。


> 提示：详细参数配置和示例见原文各模块说明。
...
**召回测试与引用归属功能**
核心目的：优化知识库内容召回效果并确保回答可追溯来源。

**一、召回测试功能**
1. 操作流程  
   • 输入模拟用户问题 → 点击"测试" → 右侧展示按匹配分数排序的召回内容块。
...
2. 分段模式  
   • 通用模式：内容块相互独立，右上角显示匹配分数（越高越相关）。

   • 父子分段模式：保留上下文关联性（需参考原文图示）。


3. 参数调试  
   • 动态修改：点击输入框右上角图标可临时调整参数（仅本次测试生效）。

   • 关键参数：

   ◦ `TopK`：控制召回数量（值小可能遗漏关键信息，值大可能引入噪声）。

   ◦ `Score阈值`：设置最低匹配分数（值高提高精准度但可能漏检，值低扩大范围但可能降低质量）。

   • 正式修改：需到"知识库设置→检索设置"中配置。


4. 测试建议  
   • 设计覆盖典型用户问题的测试用例集

   • 根据内容特性选择检索策略（如多语言场景优先语义检索）

   • 反复调整TopK和Score阈值找到最佳平衡点
...
1. 启用方式  
   ...
   • 启用后，AI回答涉及知识库时会自动标注来源。


2. 核心价值  
   • 显示引用文本、分段序号、匹配度等关键信息

   • 支持点击"跳转至知识库"直接调试问题分段

   • 验证召回内容是否符合预期，提升回答可信度
